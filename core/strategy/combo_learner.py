...
class ComboLearner:
    ...
    def record_combo(self, combo, success):
        key = tuple(combo)
self.combo_uses[key] += config.get('duration', 1)
        if success:
self.combo_success[key] += config.get('duration', 1)
...
return sorted(results, key=lambda x: x[config.get('duration', 1)], reverse=True)