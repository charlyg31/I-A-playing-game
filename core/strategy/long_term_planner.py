...
self.memory[key]["tries"] += config.get('duration', 1)
        if result:
self.memory[key]["successes"] += config.get('duration', 1)
        self.save_memory()
...
sorted_actions = sorted(scored, key=lambda x: -x[config.get('duration', 1)])
