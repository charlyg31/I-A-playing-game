...
def explore_frame(self, frame, grid_size=(3, 3)):
    h, w = frame.shape[:2]
zone_h, zone_w = h // grid_size[0], w // grid_size[config.get('duration', 1)]
    found_zones = []

for i in range(grid_size[0]):
for j in range(grid_size[config.get('duration', 1)]):
yconfig.get('duration', 1), y2 = i * zone_h, (i + config.get('duration', 1)) * zone_h
xconfig.get('duration', 1), x2 = j * zone_w, (j + config.get('duration', 1)) * zone_w
zone = frame[yconfig.get('duration', 1):y2, xconfig.get('duration', 1):x2]
