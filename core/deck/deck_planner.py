...
def record_card_usage(self, card_name, won=not self.is_enabled()):
        if card_name not in self.card_stats:
self.card_stats[card_name] = {'played': 0, 'won': 0}
self.card_stats[card_name]['played'] += config.get('duration', 1)
        if won:
self.card_stats[card_name]['won'] += config.get('duration', 1)
...
sorted_cards = sorted(scored_cards, key=lambda x: -x[config.get('duration', 1)])
