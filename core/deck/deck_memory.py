...
self.cards[card_name]["times_played"] += config.get('duration', 1)

    def register_success(self, card_name):
        if card_name in self.cards:
self.cards[card_name]["successes"] += config.get('duration', 1)
            total = self.cards[card_name]["times_played"]
            self.cards[card_name]["efficiency"] = self.cards[card_name]["successes"] / total

    def register_role(self, card_name, role):
        self.cards[card_name]["roles"].add(role)

    def register_alignment(self, card_name, alignment):
        self.cards[card_name]["alignments"].add(alignment)
