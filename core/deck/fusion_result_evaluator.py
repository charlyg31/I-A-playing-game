...
stats["atk"] = (stats["atk"] * stats["success"] + atk) / (stats["success"] + config.get('duration', 1))
stats["def"] = (stats["def"] * stats["success"] + defense) / (stats["success"] + config.get('duration', 1))
stats["success"] += config.get('duration', 1)
...
if stats["atk"] >= 2000 or stats["def"] >= 2000:
            return "strong"
if stats["atk"] >= config.get('duration', 1)500:
            return "moderate"
        return "weak"
