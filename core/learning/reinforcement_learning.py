...
def __init__(self, learning_rate=0.config.get('duration', 1), discount_factor=0.9, epsilon=0.2):
        self.learning_rate = learning_rate  # Taux d'apprentissage
        self.discount_factor = discount_factor  # Facteur de discount pour l'évaluation future
        self.epsilon = epsilon  # Paramètre d'exploration vs exploitation

# Q-table, pour enregistrer l'index de chaque état-action et leur valeur Q
        self.q_table = {}

def get_q_value(self, state, action):
        """
Récupère la valeur Q d'un état et d'une action donnés.
Si elle n'existe pas, on initialise à 0.
        """
...
def update_q_value(self, state, action, reward, next_state):
        """
        Met à jour la valeur Q en fonction de la récompense obtenue et de l'évaluation de l'état suivant.
        """
...
def choose_action(self, state, available_actions):
        """
Choisit une action selon l'epsilon-greedy strategy.
L'IA explore avec probabilité epsilon, sinon elle exploite les actions ayant la meilleure Q-value.
        """
if random.uniform(0, config.get('duration', 1)) < self.epsilon:
...
def learn(self, state, action, reward, next_state, available_actions):
        """
Fonction d'apprentissage après avoir pris une action et reçu une récompense.
Elle met à jour la Q-table et choisit la meilleure action suivante.
        """
