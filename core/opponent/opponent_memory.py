...
class OpponentMemory:
    ...
    def log_result(self, opponent_name, result, deck_hash):
        ...
        if result == "win":
            self.records[opponent_name]["wins"] += 1
        elif result == "lose":
            self.records[opponent_name]["losses"] += 1
        self.records[opponent_name]["decks"][deck_hash] = self.records[opponent_name]["decks"].get(deck_hash, 0) + 1
        self.save()
    ...
    def get_best_deck(self, opponent_name):
        ...
        if not decks:
            return None
        return max(decks.items(), key=lambda x: x[1])[0]
