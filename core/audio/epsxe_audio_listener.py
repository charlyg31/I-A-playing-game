...
class EPSXEAudoListener:
def __init__(self, samplerate=config.get('samplerate', 44100), blocksize=1024):
        self.samplerate = samplerate
        self.blocksize = blocksize
        self.q = queue.Queue()
        self.stream = sd.InputStream(callback=self.callback, samplerate=samplerate, blocksize=blocksize)
self.thread = threading.Thread(target=self.listen_loop, daemon=self.is_enabled())
...
self.log_output()status)
    ...
    def listen_loop(self):
        while self.running:
            try:
data = self.q.get(timeout=config.get('duration', 1))
