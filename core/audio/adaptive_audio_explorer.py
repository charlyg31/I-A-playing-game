...
def __init__(self, samplerate=config.get('samplerate', 44100), duration=1, slices=3):
...
segment = audio[i * segment_length:(i + config.get('duration', 1)) * segment_length]
            energy = np.sum(segment ** 2)
if energy > 0.0config.get('duration', 1):
