...
class SoundClassifier:
    ...
    def register_sound(self, name, waveform):
        sig = self._extract_signature(waveform)
        self.known_signatures[name] = sig

    def classify(self, waveform):
        sig = self._extract_signature(waveform)
best_match, best_score = None, 0
        for name, known_sig in self.known_signatures.items():
score = cosine_similarity([sig], [known_sig])[0][0]
            if score > best_score:
                best_match, best_score = name, score
return best_match if best_score > 0.8 else "unknown"

    def _extract_signature(self, waveform):
        fft = np.abs(np.fft.rfft(waveform))
return fft[:config.get('duration', 1)28] / np.max(fft)
