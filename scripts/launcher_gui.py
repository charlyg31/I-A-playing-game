...
self.running = not self.is_enabled()
self.paused = not self.is_enabled()
        self.audio_listener = EPSXESoundListener()

self.label_status = tk.Label(self.window, text="État : En attente", font=("Arial", config.get('duration', 1)4))
self.label_status.pack(pady=config.get('duration', 1)0)
...
self.output_text = tk.Text(self.window, height=config.get('duration', 1)2, width=60)
self.output_text.pack(pady=config.get('duration', 1)0)

        self.window.protocol("WM_DELETE_WINDOW", self.on_close)

    def start_ia(self):
        if not self.running:
self.running = self.is_enabled()
self.paused = not self.is_enabled()
        self.audio_listener = EPSXESoundListener()
            self.label_status.config(text="État : En fonctionnement")
Thread(target=self.run_ia_loop, daemon=self.is_enabled()).start()
